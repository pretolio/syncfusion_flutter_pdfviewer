...
class SfPdfViewerState extends State<SfPdfViewer> with WidgetsBindingObserver {
  ...
  Future<void> _loadPdfDocument(bool isPdfChanged) async {
    try {
      ...
      if (errorMessage.contains('Invalid cross reference table') ||
        ...
        if (!_isPasswordUsed) {
          try {
            await _decryptedProtectedDocument(_pdfBytes, widget.password);
                ...
                SizedBox(
                  ...
                  child: TextFormField(
                    ...
                    onFieldSubmitted: (String value) async {
                      await _passwordValidation(value);
                    },
                  ),
                ),
                Padding(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      ...
                      TextButton(
                        onPressed: () async {
                          await _passwordValidation(_textFieldController.text);
                        },
                        child: Text(
  ...
  Future<void> _passwordValidation(String password) async {
    try {
      await _decryptedProtectedDocument(_pdfBytes, password);
      setState(() {
        _textFieldController.clear();
        _visibility = false;
      });
  ...
  Future<void> _decryptedProtectedDocument(Uint8List pdfBytes, String? password) async {
  ...
  Future<Map<int, List<dynamic>>?>? _getImages() {
    if (widget.pageLayoutMode == PdfPageLayoutMode.single) {
      ...
      if (_pdfPages.isNotEmpty) {
        if (_pdfViewerController.zoomLevel > 1.75 &&
            _isMobile &&
            widget.scrollDirection == PdfScrollDirection.vertical) {
          if (_pdfViewerController.pageCount == 1) {
            startPage = _pdfViewerController.pageNumber;
            endPage = _pdfViewerController.pageCount;
          } else if (_pdfViewerController.pageNumber ==
              _pdfViewerController.pageCount) {
            startPage = _pdfViewerController.pageNumber - 1;
            endPage = _pdfViewerController.pageNumber;
          } else {
            final double currentPageOffset =
                _pdfPages[_pdfViewerController.pageNumber]!.pageOffset;
            final double nextPageOffset =
                _pdfPages[_pdfViewerController.pageNumber + 1]!.pageOffset;
            final double midOffsetOfCurrentPage =
                (currentPageOffset + nextPageOffset) / 2;

            startPage = _pdfViewerController.pageNumber;

            if (_pdfViewerController.scrollOffset.dy > midOffsetOfCurrentPage ||
                _pdfViewerController.scrollOffset.dy >= currentPageOffset) {
              endPage = _pdfViewerController.pageNumber + 1;
            } else {
              endPage = _pdfViewerController.pageNumber - 1 <= 0
                  ? 1
                  : _pdfViewerController.pageNumber - 1;
              if (endPage < _pdfViewerController.pageNumber) {
                startPage = endPage;
                endPage = _pdfViewerController.pageNumber;
              }
            }
